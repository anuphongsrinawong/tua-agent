...
@app.command()
def init(
    global_flag: bool = typer.Option(False, "--global", help="Create user-global config only"),
    project: bool = typer.Option(False, "--project", help="Create project config only"),
):
    """Initialize Tua configuration files."""
    cwd = Path.cwd()
    
    targets = []
    if not project and not global_flag:
    ...
    for target in targets:
        if target == "global":
            cfg_path = Path.home() / ".tua" / "config.toml"
        else:
            cfg_path = cwd / ".tua" / "config.toml"
        
        if cfg_path.exists():
            typer.echo(f"   ⏭️  {cfg_path} already exists")
            continue
        
        cfg_path.parent.mkdir(parents=True, exist_ok=True)
        cfg_path.write_text(
            '# Tua Agent configuration\n'
            '# Generated by `tua init`\n\n'
            '[profile]\n'
            'default = "rustacean"\n\n'
            '[tools]\n'
            'timeout = 600\n'
    ...
    typer.echo(f"\n🦀  Config initialized. Use `tua config show` to view.")
